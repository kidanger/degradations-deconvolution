...
SMART_PARAMETER(int, NORMALIZE_INPUT, 0);

SMART_PARAMETER(double, RESET_PROBABILITY, 0.005);
SMART_PARAMETER(double, MUTATION_SIGMA, 4.);
SMART_PARAMETER(float, REG_WEIGHT, 0.0008f);
SMART_PARAMETER(int, ITERATIONS, 300);
SMART_PARAMETER(double, ED_INIT, 0.025);
SMART_PARAMETER(double, CLIP, 0.);
SMART_PARAMETER(double, QUANTIZE, 0);
SMART_PARAMETER(double, GAMMA, 0);
SMART_PARAMETER(double, GAMMA_TV, 0);

SMART_PARAMETER(int, FAST, 1);
SMART_PARAMETER(int, MULTISCALE, 1);
...
void mutate(int& x, int& y, int& l, int w, int h, int d)
{
    if (drand48() > RESET_PROBABILITY()) {
        std::normal_distribution<double> distribution(0., MUTATION_SIGMA());
        int xx = x;
        int yy = y;
        do {
...
void tv_stochastic_deconvolution(img_t<T>& u, img_t<T> f, const img_t<T>& K, T lambda)
{
    ...
    if (GAMMA() > 0.) {
        if (CLIP() > 0. && QUANTIZE() > 0.) {
            gamma_quantize_clip_conv_data<T> data(K, u, QUANTIZE(), CLIP(), GAMMA());
            auto p = problem_t<T, decltype(data), decltype(regularizer)>(data, regularizer, f, u);
            stochastic_deconvolution(u, p, f.w, f.h, f.d);
        } else if (QUANTIZE() > 0.) {
            gamma_quantize_conv_data<T> data(K, u, QUANTIZE(), GAMMA());
            auto p = problem_t<T, decltype(data), decltype(regularizer)>(data, regularizer, f, u);
            stochastic_deconvolution(u, p, f.w, f.h, f.d);
        } else if (CLIP() > 0.) {
            gamma_clip_conv_data<T> data(K, u, CLIP(), GAMMA());
            auto p = problem_t<T, decltype(data), decltype(regularizer)>(data, regularizer, f, u);
            stochastic_deconvolution(u, p, f.w, f.h, f.d);
        } else {
            gamma_conv_data<T> data(K, u, GAMMA());
            auto p = problem_t<T, decltype(data), decltype(regularizer)>(data, regularizer, f, u);
            stochastic_deconvolution(u, p, f.w, f.h, f.d);
        }
...
int main(int argc, char** argv) {
    if (argc != 4)
        return std::cerr << "Usage: " << argv[0] << " input kernel output" << std::endl, 1;
    ...
    img_t<float> u;
    if (MULTISCALE()) {
        tv_stochastic_deconvolution_rec(u, f, K, REG_WEIGHT());
    } else {
        tv_stochastic_deconvolution(u, f, K, REG_WEIGHT());
    }
